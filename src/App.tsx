...
export default function App() {
  ...
  // Income input (local) synced with selected month
  const [incomeInput, setIncomeInput] = useState(
    data[selectedMonth]?.income || 0
  );
  useEffect(
    () => setIncomeInput(data[selectedMonth]?.income || 0),
    [selectedMonth, data]
  );
  ...
  // Helper: month expenses safe
  const monthExpenses = data[selectedMonth]?.expenses || [];

  // Save income to the month's data
  ...
  const handleAddOrUpdateExpense = () => {
    ...
    setData((prev) => {
      ...
      if (editingExpId) {
        ...
        return {
          ...prev,
          [selectedMonth]: { ...month, expenses: [...expenses, newExp] },
        };
      }
    });
    setExpTitle("");
  ...
  const handleDeleteExpense = (id) => {
    setData((prev) => {
      ...
      return {
        ...
        [selectedMonth]: {
          ...month,
          expenses: (month.expenses || []).filter((e) => e.id !== id),
        },
      };
    });
  };
  ...
  const handleSaveEditedCategory = () => {
    ...
    // replace category name in categories
    setCategories((prev) => prev.map((c) => (c === editingCategory ? v : c)));

    // update expenses across all months where this category is used
    setData((prev) => {
      const copy = JSON.parse(JSON.stringify(prev));
      Object.keys(copy).forEach((m) => {
        copy[m].expenses = (copy[m].expenses || []).map((ex) =>
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="bg-white rounded-2xl shadow-md p-4 mb-6">
        <h2 className="text-lg font-semibold mb-2">
          {editingExpId ? "Edit Expense" : "Add Expense"}
        </h2>
      ...
      <div className="bg-white rounded-2xl shadow-md p-4 mb-6">
        ...
        {(data[selectedMonth]?.expenses || []).length === 0 ? (
          <p className="text-gray-500">No expenses yet.</p>
        ) : (
          <ul className="divide-y">
            {(data[selectedMonth]?.expenses || []).map((e) => (
